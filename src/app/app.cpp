...
void statusPrintTask(uint32_t nowMs) {
    ...
    Serial.printf(
        "[status] wifi=%s ip=%s ws=%s mode=%s obs_front=%d batt=%d%% "
        "voltage=%.2fV current=%.2fA\n",
        WifiManager::isConnected() ? "ok" : "no", WifiManager::ip().c_str(),
        WsControlClient::isConnected() ? "ok" : "no",
        (state.driveMode() == RobotHttpServer::DriveMode::IDLE)     ? "IDLE"
        : (state.driveMode() == RobotHttpServer::DriveMode::MANUAL) ? "MANUAL"
                                                                    : "AUTO",
        drive.obstacleFrontActive() ? 1 : 0, sensors.batteryLevel(),
...
void setup() {
    ...
    Wire.begin(static_cast<int>(BoardPins::I2C_SDA),
               static_cast<int>(BoardPins::I2C_SCL));
    ...
    Serial.printf(
        "[rc522] init %s (ver=0x%02X ss=%d rst=%d sck=%d miso=%d mosi=%d)\n",
        rok ? "ok" : "fail", sensors.rfidVersion(),

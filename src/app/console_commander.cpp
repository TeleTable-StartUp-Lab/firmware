...
ConsoleCommander::ConsoleCommander(DriveController &driveRef,
                                   SensorSuite &sensorsRef,
                                   LedController &ledsRef, I2sAudio &audioRef)
    : drive(driveRef), sensors(sensorsRef), leds(ledsRef), audio(audioRef) {}

void ConsoleCommander::begin() { console.begin(); }
...
void ConsoleCommander::printHelp() {
    ...
    Serial.println("  r <v>                     - set right motor [-1.0..1.0] "
                   "(direct, no smoothing)");
    Serial.println(
        "  tank <throttle> <steer>   - set targets [-1.0..1.0] (smoothed)");
    Serial.println("  stop                      - stop both motors");
    Serial.println("  drivedbg on|off            - drive debug prints");
    Serial.println(
        "  ir                         - print IR sensor states once");
    Serial.println("  irwatch on|off              - print IR edge events");
    Serial.println(
        "  irperiodic on|off           - periodic IR snapshot every 500ms");
    Serial.println(
        "  lux                        - print light sensor lux once");
    Serial.println(
        "  luxperiodic on|off          - periodic lux print every 500ms");
    Serial.println("  imu                        - print MPU6050 values once");
    Serial.println(
        "  imuperiodic on|off          - periodic MPU6050 print every 500ms");
    Serial.println(
        "  power                      - print INA226 battery/current once");
    Serial.println(
        "  powerperiodic on|off        - periodic INA226 print every 500ms");
    Serial.println("  rfid                       - print last RFID card once");
    Serial.println(
        "  rfidwatch on|off           - print RFID reads automatically");
    Serial.println("  led on|off                 - manual LED on/off");
    Serial.println(
        "  ledauto on|off             - enable/disable auto LED control");
    Serial.println("  ledcolor <r> <g> <b>       - set LED color (0..255)");
    Serial.println(
        "  ledbri <x>                 - set LED brightness (0..255)");
    Serial.println("  vol <0..100>               - set audio volume percent");
    Serial.println("  beep                       - play short test beep");
    Serial.println(
        "  beep <hz> <ms>             - play beep with frequency and duration");
...
void ConsoleCommander::printBackendPing() {
    ...
    const uint32_t packetLossPct =
        (result.transmitted > 0)
            ? ((result.transmitted - result.received) * 100U) /
                  result.transmitted
            : 0;

    Serial.printf("%lu packets transmitted, %lu received, %lu%% packet loss, "
                  "time %lums\n",
